...
public:
	...
	{
		// SJ: i is the index in the flattend 3D-Array of values for the sampled points.
	...
	inline Vector3d pos(int i, int j, int k) const
	{
		...
		// SJ: Alternative e.g. for coord[0]:
		// SJ: coord[0] = min[0] + diag[0]*double(i)*ddx;
		// SJ: coord[0] = posX(i);
	...
	{
		// SJ: Determine index for flattened 3D-Array (same principle as with 2D-Arrays idx = y * width + x)
